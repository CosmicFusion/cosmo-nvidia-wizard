...
builder = Gtk.Builder()
builder.add_from_file("/etc/nobara/scripts/cosmo-nvidia-wizard/process.ui")
...
if (sys.argv[1]) == "install": 
    def install():
        ...
        terminal.spawn_sync(
            ...
            os.environ['HOME'],
            ["/etc/nobara/scripts/cosmo-nvidia-wizard/install.sh"],
            [],
...
if (sys.argv[1]) == "remove": 
    def remove():
        progress_bar.pulse()
        progress_bar.set_pulse_step(100.0)
        action_text.set_label("Removing...")
        terminal.spawn_sync(
            Vte.PtyFlags.DEFAULT,
            os.environ['HOME'],
            ["/etc/nobara/scripts/cosmo-nvidia-wizard/remove.sh"],
            [],
            GLib.SpawnFlags.DO_NOT_REAP_CHILD,
            None,
            None,
